    ...
    if(radios.length==5){
        ...
        for(index=0;index<respuestas.length;index++){  
            ...
            if(respuestas[index]==selecion){
                contador+=1;
            correcto+= `Pregunta ${index+1} opcion Elegida ${selecion} Correcta: ${respuestas[index]} \n`  
            }else{
              correcto+= `Pregunta ${index+1} opcion Elegida ${selecion} Icorrecta: ${respuestas[index]} \n` 
            }
            salida.innerText=correcto;
        }
    }else{
        salida.innerText="Selecione todos los Campos";
    }
